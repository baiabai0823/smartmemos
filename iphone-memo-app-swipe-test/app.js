...
const IS_NATIVE_IOS = window.Capacitor?.getPlatform?.() === "ios" || new URLSearchParams(location.search).get("ios-test") === "1";
...
function renderNoteCard(note) {
  ...
  const swipeOpen = ui.swipe?.noteId === note.id ? ui.swipe.offset : 0;
  const swipeConfirm = ui.swipe?.confirmId === note.id;
  const swipeRight = swipeOpen > 0;
...
function updateMemoDrag(event) {
  ...
  if (rootDrop && ui.view === "home") {
    ui.drag.overFolderId = "root";
...
function finishMemoDrag(event) {
  ...
  if (blockedMessage) showToast(blockedMessage, 2000);
}
...
if (action === "back") {
  ...
  if (action === "card-interaction") {
    state.settings.cardInteraction = target.dataset.value === "swipe" ? "swipe" : "longpress";
    ui.swipe = null;
    ui.longPressNoteId = null;
    scheduleSave();
    render();
...
function keepEditorCaretVisible() {
  ...
  run();
  clearTimeout(ui.caretFollowTimer);
...
app.addEventListener("pointerdown", (event) => {
  ...
  if (noteCard) {
    ...
    ui.swipe = state.settings.cardInteraction === "swipe"
      ? { noteId: note.id, startX: event.clientX, startY: event.clientY, offset: 0, axis: null, moved: false }
      : null;
    ui.notePan = {
      scroll: noteCard.closest(".scroll"),
...
app.addEventListener("pointermove", (event) => {
  ...
  if (ui.swipe && !ui.drag && !ui.longPressNoteId && !ui.longPressFolderId) {
    const sx = event.clientX - ui.swipe.startX;
    const sy = event.clientY - ui.swipe.startY;
    if (!ui.swipe.axis && Math.hypot(sx, sy) > 8) ui.swipe.axis = Math.abs(sx) > Math.abs(sy) ? "x" : "y";
    if (ui.swipe.axis === "y") {
      clearTimeout(ui.pressTimer);
      ui.pendingDrag = null;
      ui.swipe = null;
